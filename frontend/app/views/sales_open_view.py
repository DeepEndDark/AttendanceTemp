...
class SalesOpenView(tk.Frame):
    """
    Sales role: shows only open sales logs tied to currently active clients.
    Can open a new sale, add/remove items, close a sale.
    """
    ...
    def _build(self):
        ...
        tk.Label(bar, text="Open Sales", font=("", 14, "bold"), bg="white").pack(side="left")
        ...
        # ── Split: left = sale list, right = item cart ────────
        ...
        self._sale_tree = ttk.Treeview(left, columns=cols, show="headings", selectmode="browse")
        for col, txt, w in [("uid", "UID", 60), ("client", "Client", 140), ("total", "Total", 80)]:
            self._sale_tree.heading(col, text=txt)
            self._sale_tree.column(col, width=w, anchor="center")
        sb = ttk.Scrollbar(left, orient="vertical", command=self._sale_tree.yview)
        self._sale_tree.configure(yscrollcommand=sb.set)
        self._sale_tree.grid(row=0, column=0, sticky="nsew")
        sb.grid(row=0, column=1, sticky="ns")
        self._sale_tree.bind("<<TreeviewSelect>>", self._on_sale_select)
        ...
        self._item_cb = ttk.Combobox(add_bar, textvariable=self._item_var, width=18, state="readonly")
        self._item_cb.pack(side="left", padx=4)
        tk.Label(add_bar, text="Qty:", bg="white").pack(side="left", padx=(8, 2))
        self._qty_var = tk.StringVar(value="1")
        tk.Entry(add_bar, textvariable=self._qty_var, width=5).pack(side="left")
        tk.Button(add_bar, text="Add", command=self._add_item,
                  bg="#0F6E56", fg="white", relief="flat", padx=8).pack(side="left", padx=6)
        tk.Button(add_bar, text="Remove", command=self._remove_item,
                  relief="flat", padx=8).pack(side="left")

        # Item table
        icols = ("item", "qty", "subtotal")
        self._item_tree = ttk.Treeview(right, columns=icols, show="headings", selectmode="browse")
        for col, txt, w in [("item", "Item", 160), ("qty", "Qty", 60), ("subtotal", "Subtotal", 90)]:
        ...
        self._item_tree.grid(row=1, column=0, sticky="nsew", padx=(8, 0), pady=(0, 4))
        isb.grid(row=1, column=1, sticky="ns")

        self._total_lbl = tk.Label(right, text="Total: 0.00", font=("", 11, "bold"),
                                   bg="white", anchor="e")
        self._total_lbl.grid(row=2, column=0, sticky="e", padx=12, pady=6)

        self._status = tk.Label(self, text="", fg="gray", bg="white", anchor="w")
        self._status.grid(row=2, column=0, sticky="ew", padx=16, pady=4)

        self.refresh()

    def refresh(self):
        self._load_items_catalogue()
        self._load_open_sales()

    def _load_items_catalogue(self):
        try:
            items = api.list_items()
            names = [i["item_name"] for i in items if i.get("available_stock", 0) > 0]
    ...
    def _load_open_sales(self):
        ...
        try:
            # Only show sales for currently active (timed-in) clients
            active = {c["client_name"] for c in api.list_active_clients()}
            sales = api.list_open_sales()
            shown = 0
            for s in sales:
                if s["client_name"] in active:
                    self._sale_tree.insert("", "end", iid=str(s["sales_uid"]), values=(
                        s["sales_uid"], s["client_name"], f"{s['total_price']:.2f}"))
                    shown += 1
            self._status.config(text=f"{shown} open sale(s) for active clients")
        except APIError as e:
            self._status.config(text=str(e))

    def _on_sale_select(self, _=None):
    ...
    def _load_cart(self):
        ...
        try:
            sales = api.list_open_sales()
            sale = next((s for s in sales if s["sales_uid"] == self._selected_uid), None)
            if not sale:
                return
            for si in sale.get("item_list", []):
                self._item_tree.insert("", "end", values=(
                    si["item_name"], si["item_qty"], f"{si['item_total_price']:.2f}"))
    ...
    def _open_sale(self):
        ...
        if dlg.result:
            try:
                ...
                self._status.config(text=f"Opened sale #{sale['sales_uid']} for {dlg.result}")
                self.refresh()
    ...
    def _close_sale(self):
        if self._selected_uid is None:
            ...
            return
        if not messagebox.askyesno("Confirm", f"Close sale #{self._selected_uid}? Stock will be deducted."):
            return
        try:
            api.close_sale(self._selected_uid)
            self._status.config(text=f"Sale #{self._selected_uid} closed.")
    ...
    def _update_sale_row(self, sale: dict):
        uid_str = str(sale["sales_uid"])
        if self._sale_tree.exists(uid_str):
            self._sale_tree.item(uid_str, values=(
                sale["sales_uid"], sale["client_name"], f"{sale['total_price']:.2f}"))


class _PickClientDialog(tk.Toplevel):
    def __init__(self, parent, client_names: list[str]):
