...
class AccountsView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(bar, text="Account Management", font=("", 14, "bold"), bg="white").pack(side="left")
        ...
        try:
            ...
            for a in accounts:
                ...
                self._tree.insert("", "end", values=(
                    a["account_name"], a["account_type"]), tags=(tag,))
    ...
    def _delete(self):
        ...
        if not name:
            return
        if messagebox.askyesno("Confirm", f"Delete account '{name}'?"):
            try:
...
class _AccountDialog(tk.Toplevel):
    def __init__(self, parent, title: str, edit_mode: bool = False):
        ...
        if not edit_mode:
            tk.Label(self, text="Account Name:").grid(row=row, column=0, sticky="w", padx=14, pady=6)
        ...
        tk.Label(self, text="Role:").grid(row=row, column=0, sticky="w", padx=14)
        self._role_var = tk.StringVar(value="sales")
        ttk.Combobox(self, textvariable=self._role_var, values=["admin", "sales"],
                     state="readonly", width=23).grid(row=row, column=1, padx=14, pady=6)
        row += 1

        tk.Label(self, text="Password:").grid(row=row, column=0, sticky="w", padx=14)
        self._pw = tk.Entry(self, show="*", width=26)
        self._pw.grid(row=row, column=1, padx=14, pady=6)
        row += 1

        if edit_mode:
            tk.Label(self, text="Leave blank to keep current password.",
                     fg="gray", font=("", 8)).grid(row=row, column=1, sticky="w", padx=14)
            row += 1

        tk.Button(self, text="Save", command=self._save, width=12).grid(
            row=row, column=0, pady=12, padx=14)
        tk.Button(self, text="Cancel", command=self.destroy, width=10).grid(
            row=row, column=1, pady=12, padx=14)
        self.wait_window()
    ...
    def _save(self):
        ...
        if self._name is not None:
            ...
            if not name or not pw:
                messagebox.showerror("Required", "Name and password are required.")
                return
