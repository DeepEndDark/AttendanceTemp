...
from fapp.views.admin_attendance_view import (
    make_searchable_combobox, update_searchable_combobox_values,
)
...
class EnrollView(tk.Frame):
    ...
    def _build(self):
        ...
        self._build_enroll_new_panel()
        self._build_reenroll_panel()
        self._build_action_panel()

    # ── Top-left: brand-new client ────────────────────────────

    def _build_enroll_new_panel(self):
        pane = tk.LabelFrame(self, text="Enroll New Client",
        ...
        tk.Button(pane, text="Enroll New",
    ...
    # ── Bottom-left: existing client re-enrollment ────────────

    def _build_reenroll_panel(self):
        pane = tk.LabelFrame(self, text="Re-enroll Existing Client",
                             bg="white", padx=14, pady=14)
        pane.grid(row=2, column=0, sticky="nsew",
                  padx=(20, 8), pady=(6, 10))
        pane.columnconfigure(1, weight=1)

        tk.Label(pane, text="Client name:", bg="white").grid(
            row=0, column=0, sticky="w", pady=5)
        self._re_name_var = tk.StringVar()
        self._re_name_cb = make_searchable_combobox(
            pane, self._re_name_var, [], width=24)
        self._re_name_cb.grid(row=0, column=1, sticky="ew",
                              padx=(8, 0), pady=5)

        tk.Label(pane, text="Subscription plan:",
                 bg="white").grid(row=1, column=0, sticky="w", pady=5)
        self._re_sub_var = tk.StringVar()
        self._re_sub_cb = ttk.Combobox(pane, textvariable=self._re_sub_var,
                                       state="readonly", width=24)
        self._re_sub_cb.grid(row=1, column=1, sticky="ew",
                             padx=(8, 0), pady=5)
        self._re_sub_cb.bind("<<ComboboxSelected>>", self._show_re_sub_detail)

        self._re_sub_detail = tk.Label(pane, text="", fg="#0F6E56",
                                       bg="white", font=("", 9),
                                       wraplength=260, justify="left")
        self._re_sub_detail.grid(row=2, column=0, columnspan=2,
                                  sticky="w", pady=(0, 8))

        tk.Button(pane, text="Re-enroll",
                  command=self._re_enroll,
                  bg="#0F6E56", fg="white",
                  relief="flat", padx=12, pady=6).grid(
            row=3, column=0, columnspan=2, pady=(4, 0))

        self._reenroll_status = tk.Label(pane, text="", fg="#0F6E56",
                                         bg="white", wraplength=280,
                                         font=("", 9))
        self._reenroll_status.grid(row=4, column=0, columnspan=2,
                                    pady=(4, 0))

        # Recent list — shows who was recently enrolled/re-enrolled
        ...
        for col, txt, w in [
            ("name", "Client", 110), ("plan", "Plan", 90),
            ("expires", "Expires", 90), ("days", "Days left", 60)
        ]:
            self._recent.heading(col, text=txt)
            self._recent.column(col, width=w, anchor="center")
        self._recent.grid(row=6, column=0, columnspan=2,
                          sticky="ew", pady=4)

        self._load_reenroll_subs()
        self._load_recent()
    ...
    def _build_action_panel(self):
        pane = tk.LabelFrame(self, text="Time-In / Time-Out & Locker",
                             bg="white", padx=14, pady=14)
        pane.grid(row=1, column=1, rowspan=2, sticky="nsew",
                  padx=(8, 20), pady=10)
        ...
        self._att_cb = make_searchable_combobox(
            pane, self._att_var, [], width=26)
    ...
    def _load_reenroll_subs(self):
        try:
            self._re_subs = api.list_subscriptions()
            names = [s["subscription_name"] for s in self._re_subs]
            self._re_sub_cb["values"] = names
            if names:
                self._re_sub_cb.current(0)
                self._show_re_sub_detail()
            else:
                self._reenroll_status.config(
                    text="No subscription plans found. Add plans first.",
                    fg="orange")
        except APIError as e:
            self._reenroll_status.config(text=f"Error loading plans: {e}",
                                         fg="red")

    def _show_re_sub_detail(self, _=None):
        name = self._re_sub_var.get()
        sub = next((s for s in self._re_subs
                    if s["subscription_name"] == name), None)
        if not sub:
            return
        trainer_txt = (f" | Trainer: {sub['trainer_duration_days']}d "
    ...
    def _load_clients(self):
        try:
            ...
            update_searchable_combobox_values(self._att_cb, names)
            if names and not self._att_var.get():
                self._att_var.set(names[0])
            # Keep the re-enroll name picker in sync with the same list
            update_searchable_combobox_values(self._re_name_cb, names)
    ...
    def _re_enroll(self):
        name = self._re_name_var.get().strip()
        sub  = self._re_sub_var.get()
        if not name or not sub:
            self._reenroll_status.config(
                text="Client name and subscription required.", fg="red")
            return

        # Validate against the loaded client list before round-tripping to
        # the backend — catches typos immediately instead of a 404 later.
        known_names = list(getattr(self._re_name_cb, "_full_values", []))
        if known_names and name not in known_names:
            self._reenroll_status.config(
                text=f"No client named '{name}' found. "
                     "Check spelling or use Enroll New above.",
                fg="red")
            return

        self._reenroll_status.config(text="Re-enrolling...", fg="#0F6E56")
        import threading
        def _worker():
            try:
                ...
                msg = str(e)
                self.after(0, lambda msg=msg: self._reenroll_status.config(
                    text=msg, fg="red"))
